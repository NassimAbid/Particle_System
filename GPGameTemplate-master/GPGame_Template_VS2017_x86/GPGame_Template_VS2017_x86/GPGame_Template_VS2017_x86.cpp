...
void startup() {
	...
	myCube.Load();
	const float rangefromcolx = 0.1;
	const float rangetocolx = 1.0;
	std::uniform_real_distribution<float>  distr_colx(rangefromcolx, rangetocolx);
	const float rangefromcoly = 0.1;
	const float rangetocoly = 1.0;
	std::uniform_real_distribution<float>  distr_coly(rangefromcoly, rangetocoly);
	const float rangefromcolz = 0.1;
	const float rangetocolz = 1.0;
	std::uniform_real_distribution<float>  distr_colz(rangefromcolz, rangetocolz);
	std::random_device    rand_dev;
	std::mt19937          generator(rand_dev());
	// We loaded our cubes
	for (int i = 0; i < MaxParticles; i++) {

		GLfloat red = (float)distr_colx(generator);
		GLfloat green = (float)distr_coly(generator);
		GLfloat blue = (float)distr_colz(generator);
		CubeBomb[i].Load();

		CubeBomb[i].fillColor = glm::vec4(red, green, blue, 1.0f);
	}
	...
	// Define cube physics
	cubePhysics.position = glm::vec3(2.0f, 0.5f, 20.0f);
